...
std::complex<double> *discretTransformFourier(std::uint32_t width, std::uint32_t height, std::uint32_t begin_line, std::uint32_t end_line, unsigned char const *pixels)
{
    ...
    for (std::uint32_t k1 = 0; k1 < ni; ++k1)
    {
        ...
        {
            for (std::uint32_t n2 = begin_line; n2 < end_line; ++n2)
            {
...
int main(int nargs, char *argv[])
{
    ...
    uint32_t local_height = height / nbp;

    std::chrono::steady_clock::time_point DFT_begin = std::chrono::steady_clock::now();
    std::complex<double> *local_fourier = discretTransformFourier(width, height, local_height * rank, local_height * (rank + 1), image);
    std::complex<double> *fourier = new std::complex<double>[width * height];

    MPI_Reduce(local_fourier, fourier, width * height, MPI_DOUBLE_COMPLEX, MPI_SUM, 0, MPI_COMM_WORLD);
